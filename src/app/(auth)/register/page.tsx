      ...
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{
            ...
            paddingLeft: 48, paddingRight: rightSlot ? 48 : 16, borderRadius: 12,
...
function PwRule({ met, text }: { met: boolean; text: string }) {
  return (
    <li style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.78rem', color: met ? '#16a34a' : '#94A3B8', transition: 'color 0.15s' }}>
      <span style={{ width: 4, height: 4, borderRadius: '50%', background: met ? '#16a34a' : '#CBD5E1', flexShrink: 0, transition: 'background 0.15s' }} />
      {text}
    </li>
  );
}
...
export default function RegisterPage() {
  ...
  const pwRules = [
    { met: password.length >= 8,       text: '8–64 characters' },
  ...
  function triggerError(msg: string, highlight: string | null = null) {
    setError(msg);
    setField(highlight);
    setShake(true);
  ...
  function clearError() { setError(''); setField(null); }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim()) {
      triggerError('Please enter your full name.', 'name'); return;
    }
    if (!email.trim()) {
      triggerError('Please enter your email address.', 'email'); return;
    }
    const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRe.test(email.trim())) {
      triggerError("That doesn't look like a valid email address.", 'email'); return;
    }
    if (!allRulesMet) {
      triggerError('Your password doesn\'t meet all the requirements below.', 'password'); return;
    }
    if (!confirmPassword) {
      triggerError('Please confirm your password.', 'confirm'); return;
    }
    if (password !== confirmPassword) {
      triggerError('Passwords do not match.', 'confirm'); return;
    }
    if (!agreed) {
      triggerError('You must agree to the Terms of Use and Privacy Policy to continue.', 'terms'); return;
    }

    setError(''); setField(null); setLoading(true);
    ...
    try {
      ...
      if (!res.ok) {
        // Map known server errors to helpful messages
        const msg: string = data.error ?? '';
        if (msg.toLowerCase().includes('already') || msg.toLowerCase().includes('exist')) {
          triggerError('An account with that email already exists. Try logging in instead.', 'email');
        } else {
  ...
  return (
    <>
      ...
      <div style={{
        minHeight: '100vh', background: 'linear-gradient(160deg, #EBF0F9 0%, #E2EAF4 100%)',
        ...
        <div
          ...
          style={{
            width: '100%', maxWidth: 520, background: '#FFFFFF', borderRadius: 20,
        ...
        >
          ...
          <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <AuthInput
              ...
              autoComplete="name" required icon={<IconPerson />}
              hasError={field === 'name'}
            />
            ...
            <AuthInput
              ...
              autoComplete="email" required icon={<IconMail />}
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: field === 'password' ? '#dc2626' : '#94A3B8', display: 'flex', alignItems: 'center', pointerEvents: 'none' }}>
                  <IconLock />
                </span>
                <input
                  ...
                  style={{
                    ...
                    borderRadius: 12, border: `1.5px solid ${field === 'password' ? '#dc2626' : '#D1D9E6'}`,
                    background: field === 'password' ? '#fef2f2' : '#F8FAFD',
                    fontSize: '0.95rem', color: '#1a2e1a', outline: 'none',
                ...
                <button type="button" onClick={() => setShowPw((v) => !v)}
                  style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', zIndex: 2, color: '#94A3B8', cursor: 'pointer', background: 'none', border: 'none', padding: '4px', display: 'flex', borderRadius: 6 }}
                  aria-label={showPw ? 'Hide password' : 'Show password'}>
                  <IconEye open={showPw} />
                </button>
              </div>

              {/* Live rules — visible once user starts typing */}
              {password.length > 0 && (
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: passwordMismatch || field === 'confirm' ? '#dc2626' : '#94A3B8', display: 'flex', alignItems: 'center', pointerEvents: 'none' }}>
                  <IconLock />
                </span>
                <input
                  id="confirm-password" type={showConfirmPw ? 'text' : 'password'} value={confirmPassword}
              ...
              {confirmPassword.length > 0 && (
                <p style={{ marginTop: 6, fontSize: '0.78rem', color: passwordsMatch ? '#16a34a' : '#dc2626', display: 'flex', alignItems: 'center', gap: 5 }}>
                  {passwordsMatch ? (
                    <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M20 6 9 17l-5-5" /></svg> Passwords match</>
                  ) : (
            ...
            {/* Terms */}
            <label htmlFor="terms" style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer', userSelect: 'none', padding: '10px 14px', borderRadius: 10, border: `1.5px solid ${field === 'terms' ? '#dc2626' : 'transparent'}`, background: field === 'terms' ? '#fef2f2' : 'transparent', transition: 'border-color 0.15s, background 0.15s' }}>
              <input
        ...
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', borderTop: '1px solid #E2E8F0', padding: '10px 16px', textAlign: 'center' }}>
          <p style={{ fontSize: '0.78rem', color: '#94A3B8', margin: 0 }}>FutureYou — Secure Financial Planning Portal</p>
        </div>
      </div>
    </>
  );
